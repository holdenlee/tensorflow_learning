...
def train(fs, step_f, output_steps=10, summary_steps=100, save_steps=1000, eval_steps = 1000, max_steps=1000000, train_dir="/", log_device_placement=False, batch_size=128,train_data=None,validation_data=None, test_data=None, feed_dict_fun = None, eval_feed={}):
  """
  Train model.
  ...
  with tf.Graph().as_default():
    ...
    for step in xrange(max_steps):
      ...
      if ((valid_pos_int(eval_steps) and (step + 1) % (eval_steps) == 0)) or (eval_steps!=None and (step + 1) == FLAGS.max_steps):
